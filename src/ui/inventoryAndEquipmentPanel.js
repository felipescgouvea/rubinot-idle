...
import { ITEMS, EQUIPMENT_SLOTS, EQUIPPABLE_TYPES, CONSUMABLE_TYPES, isRelicId, resolveEquippedItem } from '../domain/items.js?v=342';
import { RARITY_TIERS } from '../domain/rarity.js?v=328';
import { on, EVENTS } from '../shared/eventBus.js?v=329';
import { saveGame } from '../application/saveGameUseCase.js?v=331';
import { openModal, closeModal, itemIconImg, goldIconImg } from './shared.js?v=334';
...
export function openItemModal(itemId, fromBag = false) {
  ...
  openModal(`
    ...
    ${statsHtml}
    ${absorbHtml(item)}
    <p style="margin-top:8px; color:var(--ink-soft); font-size:12px">${t('inventory.sellLabel')} ${item.sell} ${goldIconImg('inline-icon')}</p>
