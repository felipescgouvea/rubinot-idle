...
import { ZONES, isZoneUnlocked, boostedZoneForDate } from '../domain/bestiary.js?v=370';
import { MONSTERS } from '../domain/bestiary.js?v=370';
import { XP_TABLE, MAX_LEVEL } from '../domain/character.js?v=378';
import { cityName } from '../domain/cities.js?v=354';
import { ITEMS } from '../domain/items.js?v=362';
import { monsterSpriteFile, spriteUrl, effectSpriteFile, missileSpriteFile, spriteImgOrFallback } from '../infrastructure/tibiaSprites.js?v=352';
import { areaMaxTargets } from '../domain/attackAreas.js?v=347';
import { on, emit, EVENTS } from '../shared/eventBus.js?v=349';
import { openModal, itemIconImg, vitalIconImg, goldIconImg, formatNum, applyHpState, hpStateClass } from './shared.js?v=354';
import { uiIcon, huntToggleIcon } from './uiIcons.js?v=352';
import { getCurrentMonster, getCurrentPack, getRecentDead, getHuntStats, isBossOnlyHunt } from '../application/huntUseCases.js?v=415';
import { MAX_BLESSINGS, blessingCost, deathXpLossPct, reviveHpPct } from '../domain/blessings.js?v=347';
import { getProjectileSpeedMs } from '../application/adminUseCases.js?v=352';
import { t } from '../i18n/i18n.js?v=367';
import { setStageWalking } from './stageWalk.js?v=188';
...
function renderBlessings() {
  ...
  const levelPct = nivel < MAX_LEVEL && XP_TABLE[nivel - 1] ? (curXp / XP_TABLE[nivel - 1]) * 100 : 0;
...
function battleListEntry({ uid, defKey, name, pct, hp, maxHp, target, dead, hit }) {
  return `<div class="battle-list-entry ${target ? 'target' : ''} ${dead ? 'dead' : ''} ${hit ? 'hit-flash' : ''}" data-uid="${uid}" data-def="${defKey}" ${!dead ? `onclick="selectTarget('${uid}')"` : ''}>
...
function updateSceneMode() {
  ...
  // Estado PARADO: o palco congelava sem vida (parecia bug/travado). Com uma zona
  // escolhida mas a caçada parada, mostra um CTA claro pra retomar — o botão do
  // rodapé passava batido. Sem zona (nada escolhido) não mostra nada.
  const stopped = !G.hunting && !!G.activeZone;
  stage.classList.toggle('scene-stopped', stopped);
  let ov = stage.querySelector('.scene-stopped-cta');
  if (!stopped) { if (ov) ov.remove(); }
  else if (!ov) {
    ov = document.createElement('div');
    ov.className = 'scene-stopped-cta';
    ov.innerHTML = `<button class="scene-resume-btn" onclick="toggleHunt()">${huntToggleIcon('play')} ${t('hunt.resumeCta')}</button>`;
...
export function playProjectile({ missile, targetUid, hitId, silent } = {}) {
  // Projétil SILENCIOSO: a magia/runa já mostrou o projétil dela no cast; este é só o
  // "relógio" da flecha básica pra a queda de HP cair no tempo certo, SEM desenhar a
  // 2ª flecha (a flecha fantasma). Não remove o projétil da magia (early return antes
  // da blindagem) e agenda o pouso na mesma duração de voo. Ver huntUseCases.
  if (silent) {
    if (hitId) setTimeout(() => emit(EVENTS.COMBAT_PROJECTILE_LANDED, { hitId }), getProjectileSpeedMs());
    return;
  }
...
function floatDamage({ uid, amount, element, onPlayer } = {}) {
  ...
  if (amount >= 800) span.classList.add('big');
