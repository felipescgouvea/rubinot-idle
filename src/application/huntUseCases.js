...
import { G, ACCOUNT } from './gameStore.js?v=358';
import { startHuntSession, stopHuntSession, getHuntState, idleHealOnServer, setHuntTarget, updateHuntRtc, getAccessToken } from '../infrastructure/authClient.js?v=366';
import { conectarRealtime, desconectarRealtime, realtimeAtivo } from '../infrastructure/realtimeClient.js?v=363';
import { ZONES } from '../domain/bestiary.js?v=377';
import { questZone } from '../domain/quests.js?v=6';
...
import { VOCATIONS, VOC_TRAINING, XP_TABLE, MAX_LEVEL, PROMOTION } from '../domain/character.js?v=385';
import { SPELLS, isSpellAvailable, defaultHealSpellId } from '../domain/spells.js?v=356';
import { canUseAttackRune, normalizeAttackSpells, isRuneEntry, runeEntryId } from '../domain/rtcConfig.js?v=388';
import { monsterAttack, equippedWeaponSkillId } from '../domain/combatFormulas.js?v=387';
import { elementMod } from '../domain/elements.js?v=354';
import { STAMINA_MAX } from '../domain/stamina.js?v=354';
import { ITEMS } from '../domain/items.js?v=369';
import { ITEM_BUY_PRICE } from '../domain/shopCatalog.js?v=357';
import { MONSTERS } from '../domain/bestiary.js?v=377';
import { RARITY_TIERS } from '../domain/rarity.js?v=355';
import { spellEffectName, spellMissileName, runeEffectName, runeMissileName, basicAttackMissile, meleeSwingName } from '../domain/combatFx.js?v=357';
import { emit, on, EVENTS } from '../shared/eventBus.js?v=356';
import { getDef, getMagic, getMaxHp, getMaxMana, getSpd } from './stats.js?v=355';
import { checkBpTier, bumpMissionProgress } from './battlePassUseCases.js?v=355';
import { saveGame } from './saveGameUseCase.js?v=358';
import { isStaminaEnabled, isConsumeAmmo, getProjectileSpeedMs } from './adminUseCases.js?v=359';
import { itemLogIcon, monsterLogIcon } from './logIcons.js?v=357';
...
export function getHuntStats() {
  ...
  return {
    ...
    loot: s.loot, lootH: perHour(s.loot),
    supplies: s.supplies, suppliesH: perHour(s.supplies),
...
// Inicia a RAID de uma quest, à prova da corrida com o reconcile do auto-resume.
// stopHuntLocalOnly() é SÍNCRONO: zera hunting/starting e mata o loop de
// reconcile (que fica reescrevendo G.activeZone com a zona do servidor). Só
// então setamos a zona sintética da quest e arrancamos — sem um reconcile
// stale sobrescrever a zona de volta pra caçada anterior.
export function startQuestRaid(questId) {
  ...
  try { console.log('[QUESTDBG] raid', questId, 'activeZone=', G.activeZone, 'hunting=', G.hunting, 'starting=', starting); } catch(e){}
...
function renderCombatEvents(events) {
  ...
  for (const ev of events) {
    ...
    if (ev.kind === 'basic') {
      ...
      // Brilho elemental NO MONSTRO ao levar a magia (fogo/gelo/energia/terra/morte/
      // sagrado — ver style.css: .monster-sprite-wrap.spell-*). O parâmetro
      // spellElement de renderMonsterDisplay existia pra isto mas nunca era
      // preenchido: só PLAYER_BATTLE_SIDE carregava o elemento (lado do boneco), e
      // o glow do monstro ficava morto. Passa em MONSTER_DISPLAY pro flash disparar.
      emit(EVENTS.MONSTER_DISPLAY, { hit: true, spellElement: ev.element });
      ...
      // Contabiliza o custo do suprimento no Hunt Analyzer: poções têm sell=0, o
      // custo real é o preço de COMPRA na loja (ver domain/shopCatalog.js). Sem
      // isto o campo "Suprimentos" ficava sempre em 0 (queixa do Felipe: mana
      // potion não era contabilizada).
      huntSession.supplies += ITEM_BUY_PRICE[ev.item] || 0;
...
function applyServerPack(pack, frontDmg = 0, frontEl = 'physical') {
  ...
  pack.forEach(m => {
    ...
    if (prev && m.hp < prev.hp) {
      ...
      // Lunge do boneco no golpe REAL — uma vez por tick, mesmo que o tick
...
export function startHunt() {
  ...
  if (G.hunting || starting) { try { console.log('[QUESTDBG] startHunt BAILOU hunting=', G.hunting, 'starting=', starting, 'zone=', G.activeZone); } catch(e){} return; }
...
// Re-sincroniza a sessão VIVA do servidor com a vocação/equipamento ATUAIS,
// recriando-a com um snapshot novo. Necessário quando o personagem GRADUA no
// meio de uma caçada (nível 8): a sessão do servidor tirou o snapshot de
// vocação no /hunt/start e lê o equipamento do banco UMA vez — /hunt/rtc só
// atualiza preferências (prioridade/cura/estilo), nunca vocação/equipamento.
// Sem isto, o servidor seguia calculando o combate com a vocação e o kit
// INICIAL (couro) até a caçada parar, mesmo o cliente já mostrando a nova
// vocação e o Graduate Set. Como grantGraduateKit já gravou o kit no banco
// ANTES desta chamada, o /hunt/start relê o equipamento certo. Reseta os
// cursores porque a sessão nova recomeça combatSeq/killSeq do zero (mesmo
// motivo de tryResumeServerSession).
export function resyncHuntSession() {
...
export function doCosmeticTick() {
  ...
  // (1) Golpe básico — o SWING do boneco e o TREMOR do monstro NÃO saem mais
  // daqui. Este tick corre no seu próprio relógio (400-2400ms/spd), solto do
  // instante do dano real; emitir o "atacar"/"apanhar" aqui fazia o boneco
  // balançar e o monstro tremer À TOA, sem relação com o golpe — parte central
  // da queixa de "combate não fluido". Agora o lunge e o tremor são disparados
  // por applyServerPack() no MESMO quadro em que a vida cai (golpe server-truth).
  // O projétil em si também NÃO dispara aqui — mesma razão. Disparar o voo daqui
  // e a queda de vida de applyServerPack() por um timeout separado eram dois
...
export function gainXp(amount) {
  G.xp += amount;
  while (G.level < MAX_LEVEL && G.xp >= XP_TABLE[G.level - 1]) {
