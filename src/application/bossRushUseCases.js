...
import { ZONES, BOSS_MONSTER_IDS, isZoneUnlocked } from '../domain/bestiary.js?v=107';
import { startHunt, stopHunt, setBossOnlyMode, isBossOnlyHunt } from './huntUseCases.js?v=107';
...
export function startBossRush(zoneId) {
  ...
  if (!isZoneUnlocked(zoneId, G.level, G.currentWorld, G.defeatedZoneBosses)) {
    emit(EVENTS.NOTIFY, { msg: 'Zona ainda não desbloqueada.', type: 'error' });
    return;
  }
