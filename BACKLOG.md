...
- [x] **auto-sell presets** (decisão 2b) — 3 presets no Settings: **Desligado** / **Lixo barato** (≤50) / **Lixo comum** (≤250, cobre o junk 55–243) + input custom. Verificado em prod (probe-autosell-presets: Lixo comum→250, liga, destaque, 0 erros). — `b66e5ba8`/`049f95fa`
...
- [~] **IMBUEMENTS — overhaul (prioridade Felipe #6+#8)**
  - [x] **(a) finalizar/cada um funciona** — AUDITADO: os 8 efeitos JÁ estão ligados no combate (arma: lifeleech/manaleech/elemental em `huntEngine:717-723`; proteção: `computePlayerAbsorb:218-226` reduz dano elemental). Valores CANÔNICOS vs `imbuements.xml` do Crystal: Vampirism 5%/Void 3%/Scorch 10% (basic) e proteções 15% (powerful/base3) — nenhum inventado.
  - [x] **(d) ícones reais** — baixados os 8 ícones reais do Tibia (`.png` do wiki → webp). Verificado em prod (shot-imbue-page: Vampirism/Void/Scorch carregam, 0 erros). — `8866fa6a`
  - [x] **(b) página estilo/cores do Tibia** — janela de imbuing redesenhada no visual do cliente (painel escuro/ardósia + moldura dourada, gems em soquete dourado, materiais em chips escuros), commitada num único visual dark. Verificado em prod (shot-imbue-page, 0 erros). — `f6053a9d`
  - [ ] **(c) imbuir item da MOCHILA** — ⚠️ **mudança de modelo no servidor**: hoje o imbuement mora por **eq_slot** (`session.imbuements[eqSlot]`), não por instância de item. Imbuir item da mochila exige guardar o imbuement **por item** (refactor de dados no servidor + persistência). Não é CSS — precisa de sessão focada
- [x] **BUG: Hunt Analyze não contabiliza suprimentos** — o acumulador `huntSession.supplies` nunca era incrementado (poções têm `sell:0`; a nota mandava usar "valor de venda" = 0). Agora soma o **preço de COMPRA** (`ITEM_BUY_PRICE` do shopCatalog) a cada poção consumida; nota corrigida (loot=venda, suprimentos=compra). Verificado: unit test do ITEM_BUY_PRICE (mana_potion=56, etc., 92 entradas) + smoke sem erros (observação ao vivo inconclusiva: char não bebeu na janela). — `447bed32`
- [ ] Sprites de monstro **cortadas** (zone picker + boss zone) `[decisão]` — **INVESTIGADO A FUNDO com o cliente instalado:** NÃO é fallback nem bug de compositing. Valkyrie (lookType 139) e Silvertoe (134) JÁ são do cliente e são **extraídas corretamente** (o grid de patterns confirma: south/base = a criatura inteira num sprite 64px). O "cortado" é a **pose NATIVA do cliente** (diagonal/dinâmica) que enche o frame 64×64 e encosta nas bordas. **Tensão de decisão do Felipe:** ele exige (1) sprite do cliente e (2) tamanho nativo 1:1 (nunca encolher/fill 82%) — mas a arte nativa dessas criaturas enche o frame, então "não encostar na borda" só é possível encolhendo (o que a regra proíbe). **Precisa Felipe decidir:** aceitar a arte nativa como está, OU permitir um padding leve só nas criaturas que enchem o frame (pequeno desvio do 1:1). Sem essa decisão, não há fix "correto" a fazer.
...
- [~] **Treino unificado** · M — ✅ **contrato feito** (`.spec/23-training.md` reescrito: 1 treino contínuo aberto-acelerado+fechado-descanso, outfit fiel, sem modo offline separado — `4c8ccd92`). **Implementação (sessão focada):** (1) **cliente** — fundir `startTraining`+`startOnlineTraining` num só `startTraining(skillId)`; remover a UI de escolher modo; painel único maior; boneco com o **outfit fiel ao equipado** (reusar o renderer de outfit do personagem). (2) **servidor** `[deploy Railway]` — o crédito precisa aplicar rate ACELERADO (10x) pro tempo com jogo ABERTO (ticks vivos, elapsed pequeno) e rate de DESCANSO (1x, cap 8h) pro gap de jogo FECHADO na MESMA sessão — hoje o rate é fixo por `training_mode`. Ajustar `server/src/index.js`/`huntEngine` pra diferenciar tick-vivo vs gap-de-resume. (3) verificar os dois rates. Arquivos: `trainingUseCases.js`, `trainingPanel.js`, `trainingStage.js`, `domain/training.js`, `server/src/*`
...
- [ ] **Sistema de Quests (raids com prêmio real)** · L — ✅ DECISÃO Felipe (5): **começar por quests SIMPLES** (não-alavanca). Cada quest = raid com começo/meio/fim; premiação **REAL de quest do Tibia** (item/acesso — NÃO inventar) ao finalizar; **1 prêmio por quest** (não repetível). Editar `.spec/` antes. Exceção documentada à regra de prêmio não-material (quests reais dão item)
