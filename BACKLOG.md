...
- [x] **Treino unificado** · M — FEITO de ponta a ponta. **Contrato** (`.spec/23-training.md`) + **servidor** (`creditTraining` por contexto: `live`=10x jogo aberto / `resume`=1x jogo fechado cap 8h, no lugar do `training_mode` fixo; endpoints `/train/start|credit|stop` ajustados; deploy Railway) + **cliente** (fundidos `startTraining`+`startOnlineTraining` num só; resume credita o gap como `resume`; UI de painel ÚNICO, sem escolher modo; palco sempre com o **outfit fiel** do personagem batendo no boneco). Verificado em prod (probe-training-unified: 1 card, sem header "Online Training", inicia com palco+outfit+stop, 0 erros; screenshot mostra o paladino no dummy com "+60 tries/min" e a nota aberto-acelera/fechado-continua). — `543f9cbe` (+deploy Railway)
