...
- [ ] Sprites de monstro **cortadas** (zone picker + boss zone) — ✅ **CONFIRMADO com o Felipe**: NÃO é CSS (tudo `object-fit:contain`, nat 64×64, sem overflow); é o **ASSET**. Valkyrie (antebraço+pés cortados) e Silvertoe (braços cortados nas 2 bordas) estão clipadas dentro do próprio webp 64×64. Bug sistêmico do reprocessamento (reescala pra ~82% mediu bbox de 1 frame e escalou demais criaturas largas/com membros estendidos). **Fix**: re-normalizar do source limpo (`assets/creature-catalog/*.gif`) com bbox = UNIÃO de todos os frames + padding garantido; passe em lote pra todas as criaturas + bump global de `MONSTER_SPRITE_VER`
