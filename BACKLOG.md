...
- [~] **(auditoria design 07-25) Trocar emojis de SO por sprites do tema** (pedido reforçado: "TODOS os ícones = Tibia real, nada inventado"). ✅ **Worlds**: os 6 mundos usavam emoji de SO → agora cada um usa uma GEMA canônica do Tibia (Yellow/Red/Green/White/Blue/Violet Gem), sprite real — `e8c8488c`, verificado. ⏳ **Falta o sweep completo**: Achievements (🌱⭐👑💀... renderizados direto), cabeçalhos do Battle Pass (⭐/🎖️), bonus do Worlds (🎁), Daily reward, e revisar os emojis que EU adicionei (level-up ⬆, resume ▶, boosted 🐾). Muitos emojis de bestiary/zona são só FALLBACK (o sprite real renderiza) — esses estão OK.
