...
- [ ] Sprites de monstro **cortadas** (zone picker + boss zone) `[asset/pipeline]` — **ROOT CAUSE (investigado com o cliente instalado):** NÃO é fallback — Valkyrie (lookType 139) e Silvertoe (134) JÁ são do cliente (a re-extração dá pixel-idêntico ao atual). O bug é no **compositing do `extract-monsters.py`**: essas criaturas têm `pw=4, ph=3, pd=2` (multi-pattern/multi-tile), e o índice do sprite (`cell = (f*pw + SOUTH)*ly`) **ignora `ph`/`pd`** → compõe os tiles errados → pose diagonal/cortada. Afeta TODAS as criaturas com ph>1 ou pd>1 (as 1×1 saíram certas). **Fix (sessão focada):** corrigir a fórmula de índice pro layout Tibia 15.x (`idx = layers*(x + pw*(y + ph*(z + pd*frame))) + layer`) + compor a grade completa w×h; testar em várias criaturas; bump global `MONSTER_SPRITE_VER`. Script de apoio: `scripts/extract-monsters-targeted.py`. Ver diretriz Regra 2.1 do spec (sprite só do cliente, sem fallback)
