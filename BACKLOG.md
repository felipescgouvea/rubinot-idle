...
- [x] ~~monstros com `hp` placeholder~~ — INVESTIGADO 2026-07-27: **não é bug de gameplay**. `audit-monstros-cs.mjs` (compara os alcançáveis ao Crystal) reporta **HP divergente = 0** e **divergências duras hp/xp/gold/loot = 0**. Todos os hp-placeholder são **dados mortos** (monstros fora de qualquer zona). A base de monstros ALCANÇÁVEIS é 100% canônica. Limpeza do dado morto é inócua/opcional. — `d36a85c0`
- [ ] **Monstros só-melee — importar os `combat` dos `.lua`** · `[server]` · `[CANON]` · precisa aval — MEDIDO 2026-07-27: dos **310 alcançáveis, só 4 têm spells; 306 são só-melee**, incluindo **189 com hp>800** que no source têm ataques elementais (Dragon/Dragon Lord/Behemoth/Demon/glooth/orc-cult etc.). É gap de fidelidade real. PORÉM adicionar spells a ~189 monstros muda o **dano de monstro em massa** (dificuldade de toda hunt → taxa de morte, sink de poção, XP/h) = classe "HP-atk em massa" que o guardrail freia. DECISÃO DO FELIPE: aprovar deslocar a dificuldade rumo à canônica (com os valores do Crystal, sem inventar)? Se sim, ataco em lotes conservadores por zona, medindo sobrevivência antes/depois.
