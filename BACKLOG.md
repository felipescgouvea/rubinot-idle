...
- [ ] Sprites de monstro **cortadas** (zone picker + boss zone) `[asset/cliente]` — CONFIRMADO que é o asset (não CSS). ⚠️ **DIRETRIZ Felipe 26/07: sprite de monstro SÓ da fonte do cliente do Tibia** (não wiki/gif). Valkyrie/Silvertoe estão com sprite de fonte não-cliente (por isso "cortadas"). **Fix**: re-extrair essas (e auditar quais outras não são client-sourced) via `scripts/extract-monsters.py` (escala nativa 1:1, NUNCA fill 82%/NEAREST — ver memória). Precisa do cliente Tibia instalado + pipeline (sessão focada). Criatura real do Tibia → sprite do cliente; custom RubinOT sem cliente → avaliar remover
