...
- [ ] 🟡 **#R5 (baixa sev) `/charm/unlock` read-modify-write sem lock** (`index.js:1118-1122`) — dois unlocks concorrentes leem os mesmos `points` e desbloqueiam 2 charms gastando além do disponível (o `points` clampa em 0 depois, mas os 2 charms ficam). Mesma classe do #R2 (corrida da carteira). Requer requisições simultâneas.
...
- [ ] 🟠 **#3 Boosts client-authoritative (exploit + UX)** — `/hunt/start` aceita `body.boosts` verbatim (`index.js:500`) e a caça usa `session.boosts` no cálculo (`huntEngine.js:500-502` xp/gold/loot ×1.5). **Exploit:** cliente forja `{xp,gold,loot: ano 3000}` → +50% permanente de graça. **UX (reportado):** boost comprado na loja no meio da caça só muda `G.boosts` local; o snapshot do hunt-start não muda → não vale até rezonar. **É all-or-nothing:** fechar o `/hunt/start` (ler `player_stats.boosts`, ignorar body — igual `prey`/`charms` já fazem) exige migrar as **3 fontes de concessão JUNTAS**, senão o boost legítimo daquela fonte para (regressão): (1) **loja** `shopUseCases.js:80` — endpoint `/shop/buy-boost` que valida `SHOP_ITEMS`, debita rubini e concede server-side (não forjável); (2) **BP** `rewardGrants.js:34` via `/bp/claim` (já valida tier server-side — só adicionar a concessão de boost, como já faz com charm no #R4); (3) **Arena** `arenaUseCases.js:172` — client-side sem endpoint, então `/boost/grant` client-trusted com cap de duração (mesma classe do `/charm/grant-bonus` de arena, que já é client-trusted). **Núcleo:** `grantBoostServer(userId,slot,type,minutes)` grava `player_stats.boosts` (acumula, padrão do daily `index.js:1926`) + atualiza `getLiveSessionBySlot().boosts` (resolve o UX na hora). **Verificação obrigatória (economia-crítico, [[verificar-visual-no-browser]]):** cada fonte — conceder boost → caçar → confirmar xp/gold/loot ×1.5 server-side (probe + Supabase). Fraqueza residual documentada: boost de arena/BP spammável (igual charm de arena hoje). `daily` já é server-authoritative.
