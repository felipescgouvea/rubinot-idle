...
- [ ] **IMBUEMENTS — overhaul (prioridade Felipe #6+#8)** — (a) **finalizar o sistema**: garantir que CADA imbuement aplique o seu efeito com o propósito certo (conferir resolução no combate do servidor); (b) **página exclusiva estilo/cores do Tibia** (janela de imbuing fiel); (c) permitir **imbuir item da MOCHILA** (não só equipado); (d) **ícones/sprites reais** dos imbuements. `src/ui/imbuementPanel.js`, `src/application/imbuementUseCases.js`, `src/domain/imbuements.js`, `server/src/*` (efeito)
