...
- [ ] Sprites de monstro **cortadas** (ex.: Valkyrie) — ⚠️ NÃO reproduz no card do Boosted: `.boosted-sprite`/`.boosted-sprite-img` usam `max-height` SEM `overflow:hidden` (não recorta); e o layout da screenshot (criatura + **nome de cidade** "Acampamento das Amazonas") não bate com o card Boosted. **Precisa Felipe apontar a tela exata** ou a Valkyrie sair como boosted (sorteio por data) pra localizar o recorte
