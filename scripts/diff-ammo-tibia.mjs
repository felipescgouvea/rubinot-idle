...
//   1. Crystal Server 1.4 (data/items/items.xml + data/weapons/weapons.xml) — dá ataque e
...
const ITEMS_XML = 'https://raw.githubusercontent.com/zimbadev/crystalserver/1.4/data/items/items.xml';
const WEAPONS_XML = 'https://raw.githubusercontent.com/zimbadev/crystalserver/1.4/data/weapons/weapons.xml';
...
// munição do Crystal Server: nome -> { id, atk, nivel }
...
for (const m of itemsXml.matchAll(/<item id="(\d+)"[^>]*name="([^"]+)"\s*>([\s\S]*?)<\/item>/g)) {
  ...
  crystal.set(nome.toLowerCase(), { id, atk: atk ? Number(atk[1]) : 0, nivel: nivelPorId[id] || 0 });
}

// TibiaWiki pro que veio depois do 1.4. Só consultamos o que o Crystal Server não conhece.
...
for (const [id, item] of nossas) {
  const real = crystal.get(nomeDe(id));
  if (real) {
    const d = [];
...
for (const [nome, d] of crystal) {
  if (!nossosNomes.has(nome)) faltando.push(`${nome.padEnd(24)} atk ${d.atk} · nível ${d.nivel}`);
}

const cab = s => `\n${'='.repeat(66)}\n${s}\n${'='.repeat(66)}`;
console.log(`nosso catálogo: ${nossas.length} munições · Crystal Server 1.4: ${crystal.size}`);
