...
- **A chance de cada raridade de Relíquia é INDEPENDENTE das outras três — não concorrem entre si.** Não precisam somar 100%: cada raridade rola a própria % isoladamente, então mais de uma pode "bater" no mesmo drop (o kill rende uma relíquia pra cada raridade que bateu) ou nenhuma pode bater.
  *Por quê:* um modelo de "escolhe só 1 raridade por peso relativo" obriga a pensar em proporção entre as quatro pra prever o resultado; com cada uma independente, o dono sabe exatamente o que uma % configurada significa, sem precisar calcular relação com as outras.
